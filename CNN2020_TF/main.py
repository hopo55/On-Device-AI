...
dataset = file_path + '/'
save_dir = 'CNN2020_TF/save/' + device + '/' + user_type
...
# Base Model
model = RT_CNN(class_num)
# optimizer_fn = tfa.optimizers.AdamW(weight_decay=1e-4, learning_rate=5e-4)
...
model_head = RT_CNN(class_num, include_top=False)
...
#-------------------------------------------------------------------------
# Model Initialize / head model last layer shape = (b, 9, 64)
# head = SoftmaxClassifierHead(batch_size, (9, 512), class_num, l2_reg=1e-4)
head = SoftmaxClassifierHead(batch_size, (9, 512), class_num)
...
with tf.Graph().as_default(), tfv1.Session() as sess:
    ...
    # head_layer -> Conv3 shape : [1, 9, 64]
    # head_variables -> fully-connected layer : [576, 16], [16]
    # predictions -> classification : [1, 16]
